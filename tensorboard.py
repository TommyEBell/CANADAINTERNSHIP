...
def write_report(reporter, data: TBData) -> None:
    ...
    result = make_grid_image(data.smilies, results.total_scores, "score", sample_size, ROWS)
    if result is not None:
        image_tensor, _ = result
        reporter.add_image(f"First {sample_size} Structures", image_tensor, step, dataformats="CHW")  # channel, height, width
